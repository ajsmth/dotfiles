...
function prCheckPrompt(context: PrContext): string {
  return [
    ...
    '- Check whether the PR has merge conflicts or is reported non-mergeable. Use gh PR metadata/status first, and inspect locally only when needed.',
    '- If merge conflicts are simple and mechanical, such as adjacent import ordering, generated lockfile context, or clearly identical one-sided edits, you may resolve them as a safe fix and report exactly what changed.',
    '- If conflict resolution requires product judgment, architecture choices, deleted-vs-restored behavior, nontrivial test expectation changes, or choosing between meaningful edits from both sides, stop and surface it in the proposed-fixes report before resolving.',
    ...
    '- Treat :pr check as interactive by default. Do not automatically make substantial code changes, rewrite architecture, update public APIs, change product behavior, adjust logging/telemetry policy, commit, push, update the PR body, or update Linear unless the user explicitly asks.',
    '- You may make simple, clearly safe fixes without asking first, such as typos, formatting/config slips, obvious import cleanup, narrow test expectation updates, or small mechanical changes that directly address a concrete finding.',
    '- For anything beyond a simple safe fix, stop before editing and present a concise proposed-fixes report for the user to choose from.',
    '- Proposed-fixes report format: one bullet per issue with severity, source (CI, human review, bot review), affected file or area, why it matters, and the proposed solution.',
    '- Keep the report short and prioritized. Group related comments into one underlying issue and omit noise, praise, stale discussion, duplicate bot metadata, and already-addressed feedback.',
    '- Avoid generating excessive PR review comments. Prefer a single concise PR summary/update comment over many inline comments unless the user explicitly asks for inline review comments.',
    '- Do not trigger a fresh CodeRabbit/Codex/bot review round from :pr check unless it is needed to verify a substantial change or the user asks for it.',
    '- If a fresh bot review round seems needed, surface that to the user right away before posting anything. Include why it is needed and that it may generate many comments or hit rate limits.',
    '- When posting any PR comment yourself, use a small comment budget: at most one summary comment by default, or at most three targeted inline comments only for distinct blocking issues.',
    '- If the work appears to require exceeding that comment budget, pause and surface the proposed comments/count to the user before posting.',
    ...
    '- If simple safe fixes were applied, run focused checks when practical and report exactly what changed. Leave commits, pushes, PR updates, and ticket updates for a follow-up instruction.',
    '- In the final response, include a short "Feedback assessed" summary that separates CI feedback from human/bot review feedback, lists safe fixes applied, and lists proposed fixes awaiting user direction.',
  ].join('\n');
}
