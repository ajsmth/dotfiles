...
function wrapperCandidates() {
    const sourceDistRelative = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../bin/llm-wrapper.js');
    const sourceTsRelative = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../bin/llm-wrapper.ts');
    ...
    return [explicit, sourceDistRelative, sourceTsRelative, cwdRelative, homeRelative].filter((value) => Boolean(value));
}
...
export async function runLlmWrapper(options) {
