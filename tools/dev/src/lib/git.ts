...
import { chat, type LlmBackend, stripMarkdownFence } from './llm.js';
...
function gh(args: string[], cwd?: string, allowFailure = false): RunResult {
...
function printNeedsInput(value: NeedsInput): void {
  printJson(value);
}

function gitRebaseContinue(repoRoot: string): RunResult {
...
function resolveForkPoint(repoRoot: string, target: string): string {
...
function commitCountSince(repoRoot: string, ref: string): number {
...
function conventionalCommitRules(subjectName: string): string[] {
  return [
...
async function readOptionalBody(options: Pick<SubmitOptions, 'body' | 'bodyFile'>): Promise<string | undefined> {
...
function fallbackCommitMessage(repoRoot: string, forkPoint: string): { subject: string; body: string } {
...
function hasUnstagedOrUntrackedChanges(repoRoot: string): boolean {
...
function fallbackSubmitContent(repoRoot: string, diffRange: string, branch: string): SubmitContent {
...
async function generateSubmitContent(
  repoRoot: string,
  branch: string,
  baseRef: string,
  diffRange: string,
  options: Pick<SubmitOptions, 'backend' | 'model'>,
): Promise<SubmitContent> {
  ...
  const diff = truncateBytes(
    gitOutput(['diff', '--no-ext-diff', '--minimal', diffRange], repoRoot),
    Number.parseInt(process.env.DEV_LLM_MAX_DIFF_BYTES ?? '40000', 10),
  );
  ...
  try {
    ...
    const parsed = JSON.parse(stripMarkdownFence(raw)) as Record<string, unknown>;
...
function managedPrBody(generatedBody: string): string {
  return [
    SUBMIT_BODY_START,
    generatedBody.trim(),
    SUBMIT_BODY_END,
...
function mergePrBody(existingBody: string, generatedBody: string): string {
...
function parsePrInfo(raw: string): PullRequestInfo {
...
function currentPr(repoRoot: string): PullRequestInfo | null {
...
function parseRepoInfo(raw: string): RepoInfo {
  const value = JSON.parse(raw) as {
    name?: unknown;
    owner?: { login?: unknown };
  };
...
function currentRepo(repoRoot: string): RepoInfo {
  return parseRepoInfo(gh(['repo', 'view', '--json', 'owner,name'], repoRoot).stdout);
}

function parseGhPaginatedJson(raw: string): unknown[] {
...
function ghApiArray(repoRoot: string, endpoint: string): unknown[] {
  const result = gh(['api', '--paginate', '--slurp', endpoint], repoRoot);
  return parseGhPaginatedJson(result.stdout);
}

function pushCurrentBranch(repoRoot: string, branch: string, remote: string): void {
...
function commentsMarkdown(payload: PrCommentPayload): string {
  return [
...
async function summarizePrComments(
...
async function summarizePrCommentsAgent(
  payload: PrCommentPayload,
  options: Pick<CommentsOptions, 'backend' | 'model'>,
): Promise<unknown> {
  const maxBytes = Number.parseInt(process.env.DEV_LLM_MAX_COMMENTS_BYTES ?? '90000', 10);
...
async function generateSquashCommitMessage(
  context: GitContext,
  options: Pick<RebaseOptions, 'backend' | 'model'>,
  ...
  const diff = truncateBytes(
    gitOutput(['diff', '--no-ext-diff', '--minimal', `${context.forkPoint}..HEAD`], context.repoRoot),
    Number.parseInt(process.env.DEV_LLM_MAX_DIFF_BYTES ?? '40000', 10),
  );
  ...
  try {
    const raw = await chat(systemPrompt, userPrompt, options);
    const parsed = JSON.parse(stripMarkdownFence(raw)) as { subject?: unknown; body?: unknown };
...
async function maybeSquash(context: GitContext, options: RebaseOptions): Promise<boolean> {
  ...
  const shouldSquash = options.squashMode === 'always'
    ? true
    : await promptYesNo(
      `Squash ${context.commitCount} ${pluralize(context.commitCount, 'commit')} into one before rebasing?`,
      false,
    );
  ...
  const message = await step('Generating squash commit message', () =>
    generateSquashCommitMessage(context, options));
...
function conflictedFiles(repoRoot: string): string[] {
...
async function resolveConflicts(context: GitContext, options: RebaseOptions): Promise<void> {
  while (await isRebaseInProgress(context.repoRoot)) {
    ...
    for (const file of files) {
      await step(`Resolving conflict in ${file}`, () =>
        resolveConflictFile(context.repoRoot, file, context, options));
    }
...
export async function rebaseOntoOrigin(options: RebaseOptions): Promise<void> {
  ...
  const rebase = await step(`Rebasing onto ${target}`, async () =>
    git(['rebase', target], repoRoot, true));
...
export async function submitChanges(options: SubmitOptions): Promise<void> {
  const repoRoot = gitOutput(['rev-parse', '--show-toplevel']);
  const branch = currentBranch(repoRoot);
  ...
  let committed = false;
  let content: SubmitContent | null = null;
  ...
  if (hasStagedChanges(repoRoot)) {
    const commitContent = options.title
      ...
      : await maybeStep(options.agent, 'Preparing commit message', () =>
        generateSubmitContent(repoRoot, branch, target, '--cached', options));
  ...
  if (existingPr) {
    ...
    const created = await maybeStep(options.agent, 'Creating pull request', async () =>
      gh(args, repoRoot));
...
export async function summarizeComments(options: CommentsOptions): Promise<void> {
  ...
  const issueComments = await maybeStep(options.agent, 'Fetching issue comments', async () =>
    ghApiArray(repoRoot, issueCommentsEndpoint));
  const reviewComments = await maybeStep(options.agent, 'Fetching review comments', async () =>
  ...
  const summary = await step('Summarizing comments with LLM', () =>
    summarizePrComments(payload, options));
