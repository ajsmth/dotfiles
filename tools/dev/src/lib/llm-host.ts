...
type Mode = 'child' | 'command' | 'confirm' | 'modal';
...
type CommandPickerEntry = {
  command: HostCommandMetadata;
  score: number;
};

type ModalState = HostModalOptions & {
  kind: 'list' | 'confirm';
  selectedIndex: number;
  scrollOffset: number;
  resolve: (result: HostModalResult) => void;
  searchText: string;
  searchMode: boolean;
  allItems: HostModalItem[];
};
...
const DEFAULT_MODAL_THEME: ModalTheme = {
  panelBg: '#2e3440',
  panelFg: '#cdcecf',
  selectedBg: '#3e4a5b',
  selectedFg: '#cdcecf',
...
function loadModalTheme(): ModalTheme {
...
function logHostEvent(event: string, details: Record<string, unknown> = {}): void {
  try {
...
function filterModalItems(items: HostModalItem[], searchText: string): HostModalItem[] {
  if (!searchText) {
    return items;
  }
...
function commandSearchScore(command: HostCommandMetadata, term: string): number {
  ...
  if (normalizedNames.some((name) => name === query)) score += 200;
  if (normalizedNames.some((name) => name.startsWith(query))) score += 120;
  if (normalizedNames.some((name) => name.includes(query))) score += 90;
...
function commandModuleRegister(module: unknown): ((host: HostExtensionApi) => Promise<void> | void) | null {
  ...
  if (typeof record.register === 'function') {
    return record.register as (host: HostExtensionApi) => Promise<void> | void;
  }

  if (typeof record.default === 'function') {
    return record.default as (host: HostExtensionApi) => Promise<void> | void;
  }
  ...
  if (record.default && typeof record.default === 'object') {
    ...
    if (typeof defaultRecord.register === 'function') {
      return defaultRecord.register as (host: HostExtensionApi) => Promise<void> | void;
    }
  }
...
function trueColor(kind: 38 | 48, value: string): string {
...
function cellSgr(cell: BufferCell, selected = false): string {
  const codes: string[] = [];
...
function renderStyledLine(
  ...
  width: number,
  selectedColumn: (column: number) => boolean = () => false,
): string {
  if (!line) {
    let output = '';
    let selected = false;
...
function selectionBounds(selection: MouseSelection): { start: BufferPoint; end: BufferPoint } {
...
function isPointSelected(selection: MouseSelection | null, point: BufferPoint): boolean {
  if (!selection) {
    return false;
  }
...
function isIncompleteEscapeSequence(value: string, start: number, end: number): boolean {
  if (end < value.length) {
    return false;
  }
...
class LlmHost {
  ...
  private readonly commands = new HostCommandRegistry();
  ...
  private commandPickerIndex = 0;
  private commandHistory: string[] = [];
  private commandHistoryIndex: number | null = null;
  ...
  private wheelFlushQueued = false;
  ...
  private hostApi(): HostExtensionApi {
    return {
  ...
  private async loadConfiguredCommands(): Promise<void> {
    let entries;
    try {
  ...
  private render(): void {
    ...
    for (let row = 0; row < bodyRows; row += 1) {
      ...
      rows.push(this.formatRow(renderStyledLine(
        buffer.getLine(bufferRow),
        this.cols,
        (column) => isPointSelected(this.selection, { row: bufferRow, column }),
      )));
    }
  ...
  private formatRow(content: string): string {
  ...
  private isAtPromptStart(): boolean {
    return this.logicalLine.length === 0 || this.terminal.buffer.active.cursorX === 0;
  }

  private modalLine(content: string, width: number): string {
  ...
  private modalSelectedLine(content: string, width: number): string {
    const innerWidth = Math.max(0, width - 4);
    const safeContent = visibleLength(content) > innerWidth
      ? truncatePlain(stripAnsi(content), innerWidth)
      : content;
  ...
  private modalBorder(left: string, fill: string, right: string, width: number): string {
    return modalPanel(modalBorderColor(`${left}${fill.repeat(Math.max(0, width - 2))}${right}`));
  }

  private modalBlank(width: number): string {
    return this.modalLine('', width);
  }

  private modalRows(width: number, maxHeight: number): string[] {
    ...
    if (filteredItems.length > 0) {
      modal.scrollOffset = Math.max(0, Math.min(
        modal.scrollOffset,
  ...
  private applyModal(rows: string[]): void {
  ...
  private commandPickerEntries(): CommandPickerEntry[] {
  ...
  private selectedCommand(): HostCommandMetadata | null {
  ...
  private navigateCommandHistory(delta: -1 | 1): boolean {
  ...
  private commandPickerRows(): string[] {
    if (this.mode !== 'command') {
      return [];
    }
    ...
    const commandWidth = Math.min(
      18,
      Math.max(...visible.map(({ command }) => command.command.length)) + 1,
    );
  ...
  private forwardPaste(content: string): void {
    if (this.selection) {
      this.selection = null;
      this.queueRender();
    }
  ...
  private isLikelyPlainPaste(data: string): boolean {
  ...
  private finishModal(action: HostModalResult['action']): void {
    const modal = this.modalState;
    if (!modal) {
      return;
    }
  ...
  private finishConfirmModal(value: boolean | 'cancel'): void {
    const modal = this.modalState;
    if (!modal) {
      return;
    }
  ...
  private handleModalInput(data: string, index: number): void {
    const char = data[index];
    const modal = this.modalState;
    if (!modal) {
      this.mode = 'child';
...
if (modal.searchMode) {
  ...
  private screenToBufferPoint(column: number, row: number): BufferPoint | null {
  ...
  private handleConfirmInput(data: string, index: number): void {
    if (!this.confirmState) {
      return;
    }
  ...
  private showModal(options: HostModalOptions): Promise<HostModalResult> {
    this.mode = 'modal';
  ...
  private confirmModal(options: HostConfirmModalOptions): Promise<boolean | 'cancel'> {
  ...
  private ensureLoadingTimer(): void {
    if (this.loadingTimer) {
      return;
    }
  ...
  private startLoading(label: string): symbol {
  ...
  private async yieldToRender(): Promise<void> {
    await new Promise<void>((resolve) => {
      setTimeout(resolve, 0);
    });
  }

  private async withLoading<T>(label: string, action: () => Promise<T> | T): Promise<T> {
  ...
  private async withSuspendedScreen<T>(action: () => Promise<T> | T): Promise<T> {
    this.exitScreen();
    try {
      return await action();
    } finally {
