...
type YesNoAnswer = boolean | 'cancel';
...
function useClackPrompts(): boolean {
...
async function promptYesNo(label: string, defaultValue: boolean): Promise<YesNoAnswer> {
  if (useClackPrompts()) {
...
async function promptYesNoRaw(label: string, defaultValue: boolean): Promise<YesNoAnswer> {
  ...
  try {
    while (true) {
      ...
      if (answer === 'enter') return defaultValue;
      if (answer === 'yes') return true;
      if (answer === 'no') return false;
      if (answer === 'abort') return 'cancel';
...
function readPromptKey(): Promise<'yes' | 'no' | 'enter' | 'abort' | 'other'> {
...
function compactPathList(values: string[], limit = 4): string {
  if (values.length === 0) {
    return dim('none');
  }
...
function prUpdatePreflightMessage(repoRoot: string, branch: string, status: string): string {
...
function printPrUpdatePreflight(repoRoot: string, branch: string, status: string, title = 'Working tree'): void {
  if (useClackPrompts()) {
...
function printCancel(message = 'Cancelled. Nothing was sent to Codex.'): void {
  if (useClackPrompts()) {
...
function commandCancelledPrompt(commandName: string): string {
  return [
...
function commandInputCancelledPrompt(): string {
  return [
...
function runPrCheck(handoffNotes: string): LocalCommandResult {
...
async function runPrUpdate(handoffNotes: string): Promise<LocalCommandResult> {
  const repoRoot = gitOutput(['rev-parse', '--show-toplevel']);
  const branch = gitOutput(['branch', '--show-current'], repoRoot);
  if (!branch) {
...
function runDiffCommand(words: string[]): LocalCommandResult {
...
async function runLocalCommand(commandLine: string): Promise<LocalCommandResult> {
...
function inject(pty: IPty, text: string, bracketedPaste: boolean): void {
  const normalized = text.trimEnd();
  const writeChunks = (value: string) => {
...
async function runLlmWrapper(options: LlmWrapperOptions): Promise<void> {
  ...
  const commandSuggestion = (): string => {
  ...
  const renderCommandPrompt = (): void => {
  ...
  const runCapturedCommand = async (commandLine: string): Promise<void> => {
    runningLocalCommand = true;
    detachInput();
    restoreTerminal();
    logger.write('local_command_start', { commandLine });
    let result: LocalCommandResult | null = null;
  ...
  const atLogicalLineStart = (): boolean => logicalLine.length === 0;
